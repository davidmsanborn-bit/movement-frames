  ...
  return new Promise((resolve) => {
    ...
    const proc = spawn(
      ...
      [
        ...
        "-an",
        "-vf",
        "fps=2,scale=320:180,signalstats,metadata=print",
        "-f",
        "null",
        "-",
      ],
    ...
    proc.on("close", (code) => {
      ...
      if (code !== 0) {
        resolve({
          ...
          wallMs,
          samples: [],
          error: `combined ffmpeg exited ${code}: ${stderr.slice(-2000)}`,
        });
        return;
      }
      resolve({
...
async function runContinuousMotionPath(
  ...
  if (path === "static_fallback") {
    console.error(
      `[detect-scenes] MOTION_SCORING_DEGRADED game=${gameId} ` +
        `combined_error=${combined.error || "none"} static_intensity=${env.staticIntensity}`,
    );
  }
